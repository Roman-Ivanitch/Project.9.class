boolean ONReconnect=true;          // Флаг который разрешает переподключение к серверу
boolean defaultServerConnect=true; // Флаг который разрешает коннектиться к серверу по умолчанию
boolean ConnectHost=true;          // Флаг который разрешает коннектится к серверу по Имени Хоста иначе по IP 
